...
interface ApiService {
    ...
    // ✅ Fetch movies based on genre(s) and sort option
    @GET("discover/movie")
    suspend fun getMoviesByGenre(
        @Query("api_key") apiKey: String,
        @Query("with_genres") genreId: String, // comma-separated string (e.g., "28,35")
        @Query("sort_by") sortBy: String = "popularity.desc" // supports: vote_average.desc, release_date.desc
    ): MovieResponse
}
